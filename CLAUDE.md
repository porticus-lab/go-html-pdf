...
go test -run 'TestCm|TestDefault|TestUniform|TestPageConfig|TestPaper|TestMargin' ./...
...
- Tests in `converter_test.go` and `example_test.go` use `package htmlpdf_test` (black-box); `page_test.go` uses `package htmlpdf` (white-box for internal helpers)
